...
### 5. 서버/구글 캘린더 간의 시간대 파싱 불일치 버그 해결 (최종 해결) 🚀
- **버그 원인**: 백엔드 호스팅 서버(예: Render.com)의 시스템 시간대가 UTC인 경우, OpenAI Realtime API 에이전트가 전달한 날짜 문자열(오프셋 없음)을 Node.js의 `new Date()`가 UTC 시간대로 임의 해석하여, 캘린더 응답(KST 변환) 시 9시간 뒤인 다음날 새벽으로 잘못 발화하여 AI 비서가 스스로 혼선을 느끼는 치명적인 시간대 불일치 버그가 있었습니다.
- **해결 로직**: 캘린더 일정 생성 요청이 완료된 후, 실제 Google Calendar API 응답 객체(`response.data.start.dateTime`)가 보장하는 **오프셋 포함 완성형 ISO 8601 문자열**을 기준으로 발화 데이터를 파싱하도록 전면 리팩토링했습니다. 이로써 서버의 시스템 타임존 설정과 무관하게 언제나 정확한 KST(Asia/Seoul) 시간으로 일정 피드백을 보장하여 UX 신뢰도를 극대화했습니다.
