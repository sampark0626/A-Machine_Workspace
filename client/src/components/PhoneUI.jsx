                    ...
                    <div key={idx} className="recent-item" onClick={onStartCall}>
                      <div className="recent-item-left">
                        <div className={`recent-item-avatar ${call.isSpecial ? 'special' : ''}`}>
                          {call.isSpecial ? <Icon name="sparkle-fill" size={18} /> : call.name[0]}
                        </div>
              ...
              <div className="keypad-container">
                ...
                <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
                  ...
                  {keypadInput.length > 0 ? (
                    <button
                      style={{ background: 'transparent', border: 'none', color: 'var(--color-label-neutral)', cursor: 'pointer', width: 50, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            ...
            {(activeTab === 'favorites' || activeTab === 'contacts') && (
              <div style={{ display: 'flex', flexDirection: 'column', flex: 1, alignItems: 'center', justifyContent: 'center', padding: 20, textAlign: 'center' }}>
                <span style={{ marginBottom: 12 }}>
                  {activeTab === 'favorites' ? <Icon name="star" size={40} style={{ opacity: 0.4 }} /> : <Icon name="person" size={40} style={{ opacity: 0.4 }} />}
                </span>
                <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--color-label-normal)', marginBottom: 6 }}>
                  {activeTab === 'favorites' ? '즐겨찾기 목록 비어있음' : '등록된 연락처가 없음'}
                </div>
                <div style={{ fontSize: 12, color: 'var(--color-label-neutral)', maxWidth: 220 }}>
        ...
        {callState === 'connecting' && (
          <div className="connecting-screen">
            <div className="connecting-avatar-container">
              ...
              <div className="connecting-avatar"><Icon name="sparkle-fill" size={52} /></div>
            </div>
          ...
          <div className="active-call-screen">
            ...
            <div className="call-content-box">
              ...
              <div className="speaking-avatar-wrapper">
                ...
                <div className="call-avatar-img"><Icon name="sparkle-fill" size={42} /></div>
              </div>
              ...
              <div className="caller-detail">
                ...
                <div className="caller-detail-timer">
                  {callMode === 'assist'
                    ? agentStatus === 'active' ? '에이전트 응답 중' : '통화 녹취 중'
                    : isModelSpeaking ? 'AI 답변 중' : '대기 중'
                  } · {formatTime(elapsed)}
                </div>
              </div>
                ...
                <>
                  ...
                  >
                    <span className="role-icon">
                      {speakerRole === 'caller' ? <Icon name="phone" size={16} /> : <Icon name="phone" size={16} />}
                    </span>
              ...
              <div className="transcript-scroll-view">
                {messages.length === 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, opacity: 0.5, textAlign: 'center', padding: '20px 10px' }}>
                    <Icon name="bubble" size={28} style={{ opacity: 0.4, marginBottom: 8 }} />
                  ...
                  <div className="glass-bubble-wrapper assistant">
                    <div className="glass-bubble assistant" style={{ padding: '8px 12px' }}>
                      <div className="typing-indicator">
                        <div className="typing-dot" style={{ background: 'var(--color-primary-normal)' }} />
                        <div className="typing-dot" style={{ background: 'var(--color-primary-normal)' }} />
                        <div className="typing-dot" style={{ background: 'var(--color-primary-normal)' }} />
                      </div>
                    </div>
            ...
            <div className="echo-guard-panel" onClick={() => onToggleEchoGuard(!echoGuard)}>
              <div className="echo-guard-title">
                <Icon name="megaphone" size={14} /> 에코 가드 모드
            ...
            <div className="active-control-panel">
              <button
                ...
                title="음소거"
              >
                <span className="ctrl-btn-icon"><Icon name={isMuted ? 'moon' : 'send'} size={20} /></span>
              ...
              {callMode === 'answering' ? (
                <button
                  ...
                  title="전화 받기"
                >
                  <span className="ctrl-btn-icon"><Icon name="phone" size={20} /></span>
                  <span>전화 받기</span>
                </button>
              ) : (
                <button
                ...
                >
                  <span className="ctrl-btn-icon"><Icon name="sparkle" size={20} /></span>
                  <span>에이전트</span>
                </button>
              )}
              ...
              <button
                ...
                title="통화 종료"
              >
                <span className="ctrl-btn-icon"><Icon name="phone" size={20} style={{ transform: 'rotate(135deg)' }} /></span>
                <span>종료</span>
              </button>
            </div>
            ...
            {showVoicePicker && (
              <div className="voice-picker-sheet">
                <div className="voice-picker-header">
                  <span>목소리 선택</span>
                  <button className="voice-picker-close" onClick={() => setShowVoicePicker(false)}><Icon name="close" size={14} /></button>
                </div>
            ...
            {showTakeoverSheet && (
              <div className="voice-picker-sheet">
                <div className="voice-picker-header">
                  ...
                  <button className="voice-picker-close" onClick={() => setShowTakeoverSheet(false)}><Icon name="close" size={14} /></button>
                </div>
                <div className="voice-picker-list">
                  <button
            ...
            {showCallKeypad && (
              <div style={{
                ...
                <button 
                  style={{ position: 'absolute', top: 30, right: 20, background: 'transparent', border: 'none', color: 'var(--color-label-normal)', fontSize: 24, cursor: 'pointer' }}
                  onClick={() => setShowCallKeypad(false)}
                >
                  <Icon name="close" size={22} />
                </button>
                <div style={{ fontSize: 22, color: 'var(--color-label-neutral)', marginBottom: 20 }}>키패드 전송</div>
                ...
                <div style={{ height: 30, color: 'var(--color-primary-normal)', fontSize: 18, fontWeight: 700 }}>
                  {keypadInput}
                </div>
              </div>
            )}
        ...
        {callState === 'ended' && (
          <div className="ended-call-screen">
            <div className="ended-status-box">
              <span className="ended-icon"><Icon name="phone" size={40} style={{ color: 'var(--color-label-neutral)', transform: 'rotate(135deg)' }} /></span>
            ...
            <div className="ended-summary-panel">
              ...
              {!summary ? (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 0', gap: 10 }}>
                  ...
                  <span style={{ fontSize: 12, color: 'var(--color-label-neutral)' }}>통화 요약 리포트를 생성하는 중입니다...</span>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div className="summary-row">
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 4 }}>
                        {summary.summary.scheduledEvents.map((evt, idx) => (
                          <span key={idx} className="summary-text" style={{ fontSize: 12, color: 'var(--color-primary-normal)' }}>
                            <Icon name="calendar" size={13} style={{ marginRight: 4, verticalAlign: 'middle' }} />{evt.title} ({evt.datetime})
                          </span>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="summary-row" style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span 
                      ...
                      style={{ marginTop: 0 }}
                    >
                      {summary.summary?.urgency === 'urgent' ? <><Icon name="triangle-exclamation-fill" size={12} style={{ marginRight: 3 }} />긴급</> : <><Icon name="circle-info" size={12} style={{ marginRight: 3 }} />일반</>}
                    </span>
                  </div>
                </div>
              )}
            ...
            <button className="ended-back-btn" onClick={onResetCallState}>
              <Icon name="refresh" size={18} /> 최근 통화 화면으로 이동
            </button>
          </div>
        )}
      </div>
